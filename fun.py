...
@app_commands.check(dm_or_guild_check)  # Apply the check here
async def welcome_card_command(interaction: discord.Interaction, user: discord.Member):
    avatar_url = user.avatar.url
    welcome_url = f"https://api.popcat.xyz/welcomecard?background=https://cdn.popcat.xyz/welcome-bg.png&text1={user.name}&text2=Welcome&text3=Member&avatar={avatar_url}"
...
async def pesti_dostanes(interaction: discord.Interaction, user: discord.User):
    # List of possible responses
    responses = [
        f"Uživatel {user.mention} dostal pěsti od {interaction.user.mention}!",
        f"{interaction.user.mention} uštědřil pěsti {user.mention}!",
        f"Tohle muselo bolet! {interaction.user.mention} dal pěsti {user.mention}.",
        f"{user.mention} právě schytal pěsti od {interaction.user.mention}!",
        f"{interaction.user.mention} rozdal pěsti {user.mention} jako mistr!",
        f"{user.mention} dostal pořádnou ránu od {interaction.user.mention}!"
    ]
